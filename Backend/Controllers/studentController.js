...
const addStudent = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json("Student Data Saved succesfully.");
    } catch (err) {
...
const getStudent = async (req, res) => {
    ...
    try {
        ...
        .sort({ name: 1})
        .limit(5)
        .select("name age date course");
        res.status(200).json(students);
    } catch (err) {
        res.status(500).json({ err: "Unable to find students data" });
    }
};
...
const updateStudent = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json("Student data has been updated successfully", student);
    } catch (err) {
